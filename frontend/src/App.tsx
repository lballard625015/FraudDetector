...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="main">
      ...
      <AssistantPanel page={view} caseId={selectedCase?.caseId} />
...
function AssistantPanel({ page, caseId }: { page: View; caseId?: string }) {
  const [question, setQuestion] = useState('What should I review on this page?')
  const [answer, setAnswer] = useState('')
  const [busy, setBusy] = useState(false)
  const ask = async () => { if (!question.trim() || busy) return; setBusy(true); setAnswer(''); try { setAnswer((await askAssistant(page, question.trim(), 'copilot', caseId)).answer) } catch (caught) { setAnswer(caught instanceof Error ? caught.message : 'Assistant unavailable') } finally { setBusy(false) } }
  return <section className="assistant-panel"><div className="assistant-header"><div><div className="assistant-title"><Sparkles size={15} /> Investigation Copilot</div><div className="panel-sub">{caseId ? 'Current case context' : `Current page: ${statusLabel(page)}`} / read-only</div></div><span className="status-badge open">Local</span></div><div className="assistant-input"><input value={question} onChange={(event) => setQuestion(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void ask() }} placeholder="Ask what is happening here" /><button className="action" disabled={busy} onClick={() => void ask()}><Send size={14} /> {busy ? 'Thinking...' : 'Ask'}</button></div>{answer && <div className="assistant-answer">{answer}</div>}</section>
}
