...
import { Activity, BarChart3, Bell, CheckCircle2, ChevronRight, CircleAlert, FileCheck2, Inbox, LayoutDashboard, Radio, Send, ShieldCheck, Sparkles, X } from 'lucide-react'
import { Account, addCaseNote, Alert, Analyst, askAssistant, askCopilot, AuthResponse, CaseEvent, CaseRecord, DashboardSummary, getAccountRisk, getAccounts, getAlerts, getCases, getDashboardSummary, getTimeline, login, logout, register, saveAuth, storedAuth, updateCaseStatus, verifyCase } from './api'
...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="main">
      ...
      <AssistantPanel page={view} caseId={selectedCase?.caseId} onAction={selectedCase ? (status) => handleStatus(status) : undefined} />
...
function AssistantPanel({ page, caseId, onAction }: { page: View; caseId?: string; onAction?: (status: WorkflowStatus) => Promise<void> }) {
  const [question, setQuestion] = useState('What should I review on this page?')
  const [answer, setAnswer] = useState('')
  const [toolsUsed, setToolsUsed] = useState<string[]>([])
  const [proposal, setProposal] = useState<{ status: WorkflowStatus; label: string; reason: string } | null>(null)
  const [busy, setBusy] = useState(false)
  useEffect(() => { setAnswer(''); setToolsUsed([]); setProposal(null); setQuestion('What should I review on this page?') }, [page, caseId])
  const ask = async () => { if (!question.trim() || busy) return; if (!caseId && page === 'cases' && /\b(escalat|investigat|resolv|dismiss)/i.test(question)) { setAnswer('Select a case first. I need a specific case context before I can propose a status change.'); return } setBusy(true); setAnswer(''); setToolsUsed([]); setProposal(null); try { const response = await askAssistant(page, question.trim(), 'copilot', caseId); setAnswer(response.answer); setToolsUsed(response.tools_used); if (response.proposed_action && ['investigating', 'escalated', 'resolved', 'dismissed'].includes(response.proposed_action.status)) setProposal(response.proposed_action as { status: WorkflowStatus; label: string; reason: string }) } catch (caught) { setAnswer(caught instanceof Error ? caught.message : 'Assistant unavailable') } finally { setBusy(false) } }
  const confirmProposal = async () => { if (!proposal || !onAction) return; await onAction(proposal.status); setProposal(null) }
  return <section className="assistant-panel"><div className="assistant-header"><div><div className="assistant-title"><Sparkles size={15} /> Investigation Copilot</div><div className="panel-sub">{caseId ? 'Current case context' : `Current page: ${statusLabel(page)}`} / plans and reads evidence / confirmation required</div></div><span className="status-badge open">Local</span></div>{page === 'cases' && !caseId && <div className="assistant-context-warning">Select a case below before asking me to investigate, escalate, resolve, or dismiss it.</div>}<div className="assistant-input"><input value={question} onChange={(event) => setQuestion(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void ask() }} placeholder="Ask what is happening here" /><button className="action" disabled={busy} onClick={() => void ask()}><Send size={14} /> {busy ? 'Planning...' : 'Ask'}</button></div>{answer && <><div className="assistant-answer-label">Investigation result</div><div className="assistant-answer">{answer}</div></>}{toolsUsed.length > 0 && <div className="assistant-tools"><span>Tools used</span>{toolsUsed.map((tool) => <code key={tool}>{tool}</code>)}</div>}{proposal && onAction && <div className="assistant-proposal"><strong>Proposed action: {proposal.label}</strong><span>{proposal.reason}</span><button className="action primary" onClick={() => void confirmProposal()}>Confirm {proposal.label}</button></div>}</section>
}
...
function CopilotPanel({ caseId }: { caseId: string }) {
  const [question, setQuestion] = useState('Why is this case worth reviewing?')
  const [answer, setAnswer] = useState('')
  const [busy, setBusy] = useState(false)
  const ask = async () => { if (!question.trim() || busy) return; setBusy(true); setAnswer(''); try { setAnswer((await askCopilot(caseId, question.trim())).answer) } catch (caught) { setAnswer(caught instanceof Error ? caught.message : 'Copilot unavailable') } finally { setBusy(false) } }
  return <section className="copilot-panel"><div className="copilot-header"><div><div className="copilot-title"><Sparkles size={15} /> Investigation Copilot</div><div className="panel-sub">Read-only evidence review</div></div><span className="status-badge open">Read only</span></div><div className="copilot-input"><input value={question} onChange={(event) => setQuestion(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void ask() }} placeholder="Ask about this case" /><button className="action" disabled={busy} onClick={() => void ask()}><Send size={14} /> {busy ? 'Reviewing...' : 'Ask'}</button></div>{answer && <div className="copilot-answer">{answer}</div>}</section>
}
